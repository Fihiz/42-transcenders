...
export class ChatGateway {
  ...
  @SubscribeMessage('getAllMyFriends')
  async getAllMyFriends(@MessageBody() emission) {
	  // demander au service Chat de recuperer la list d amis
	// emit au front la liste
	// test 
	const listFronBack = ['Bea-1', "Back-2", "Bailey-3"];
	// this.server.to(emission.socketId).emit('allMyFriends', listFronBack);

	this.server.emit('allMyFriends', listFronBack);
  }
}