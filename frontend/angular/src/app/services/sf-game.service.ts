    ...
    class Game {
      ...
      listenClick(e: MouseEvent) {
        if (
          ...
          test.game.status === 'Updating' ||
          (test.game.leftPaddle.login !== test.global.login &&
            test.game.rightPaddle.login !== test.global.login)
        ) {
          const x = Math.floor(
            (e.offsetX * test.gameCanvas.width) / test.gameCanvas.clientWidth
          );
          const y = Math.floor(
      ...
      drawEnd() {
        ...
        if (this.game.status === 'Finished') {
          ...
          if (
            ...
            if (this.game.leftPaddle.score === 10) {
              this.gameContext.fillText(
                `${this.game.leftPaddle.login} won the game !`.toUpperCase(),
                200,
                this.game.board.height / 2 + 15,
                300
              );
              ...
              this.gameContext.fillText(
                `${this.game.rightPaddle.login} won the game`.toUpperCase(),
                200,
                this.game.board.height / 2 + 15,
                300
              );
